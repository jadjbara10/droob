...
/* ═══════════════════════════════════════════════════════════════════════════
   ...
   System config · DB stats · API usage · Ads revenue · Geo data
   ═══════════════════════════════════════════════════════════════════════════ */

import React, { useEffect, useState } from "react";
import { RefreshCw, Server, Globe, Clock, Shield, Database, MapPin, DollarSign, Cpu } from "lucide-react";
    ...
    <div>
      ...
      <div style={{ display: "grid", gap: 16 }}>
        ...
        <Panel
          ...
          subtitle={adsStats ? `آخر ${adsStats.days} يوم` : "إحصائيات الإعلانات والإيرادات"}
          headerRight={<DollarSign size={16} style={{ color: "var(--accent-2)" }} />}
        >
          {adsStatsErr ? <InlineError message="فشل تحميل إحصائيات الإعلانات" onRetry={() => fetchSection(() => adsApi.getStats(30), setAdsStats, setAdsStatsErr)} />
            : adsStats ? (
              <div>
                {/* KPI Row */}
                    ...
                    <table className="data-table">
                      ...
                      <tbody>
                        {adsStats.revenueByType.map((r: any, i: number) => (
                          <tr key={i}>
                            <td>
                              <span className={`badge ${r.ad_type === "rewarded" ? "badge-purple" : r.ad_type === "interstitial" ? "badge-warn" : "badge-info"}`}>
                                {r.ad_type === "banner" ? "بانر" : r.ad_type === "interstitial" ? " interstitial " : "مكافأة"}
                              </span>
                            </td>
                ...
                {/* Revenue by Network */}
                {adsStats.revenueByNetwork?.length > 0 && (
                  <div style={{ marginBottom: 16 }}>
                    <h4 style={{ fontSize: 13, fontWeight: 600, marginBottom: 8, color: "var(--text-secondary)" }}>
                      الإيرادات حسب الشبكة الإعلانية
                    </h4>
                    <table className="data-table">
                      <thead>
                        <tr>
                      ...
                      <tbody>
                        {adsStats.revenueByNetwork.map((r: any, i: number) => (
                          <tr key={i}>
                            <td><span className="cell-mono">{r.ad_network}</span></td>
                            <td className="cell-mono">${r.total_revenue.toFixed(3)}</td>
                            <td className="cell-mono">{r.impressions.toLocaleString("ar-JO")}</td>
                          </tr>
                        ))}
                ...
                {/* Daily Revenue */}
                {adsStats.dailyRevenue?.length > 0 && (
                  <div>
                    <h4 style={{ fontSize: 13, fontWeight: 600, marginBottom: 8, color: "var(--text-secondary)" }}>
                      الإيرادات اليومية
                    </h4>
                    <table className="data-table">
                      ...
                      <tbody>
                        {adsStats.dailyRevenue.slice(-14).reverse().map((r: any, i: number) => (
                          <tr key={i}>
                            <td className="cell-mono">{r.date}</td>
                            <td className="cell-mono">${r.revenue.toFixed(3)}</td>
                            <td className="cell-mono">{r.impressions.toLocaleString("ar-JO")}</td>
                          </tr>
                        ))}
                      </tbody>
