...
const MOCK_DRIVERS: Driver[] = [
  ...
  { id: "DRV-0005", name_ar: "سامر يوسف", phone: "0795678901", license_number: "LIC-1005", license_expiry: "2028-03-22", assigned_vehicle_plate: "د 3456", assigned_vehicle_id: "VEH-0004", assigned_route_name: "باص رقم ٢٠٠", assigned_route_id: "RT-004", status: "active", created_at: "2025-04-12" },
  { id: "DRV-0006", name_ar: "طارق ناصر", phone: "0796789012", license_number: "LIC-1006", license_expiry: "2026-07-18", assigned_vehicle_plate: "ه 7890", assigned_vehicle_id: "VEH-0005", assigned_route_name: "باص رقم ٤٠٠", assigned_route_id: "RT-005", status: "inactive", created_at: "2025-02-28" },
  { id: "DRV-0007", name_ar: "رائد خالد", phone: "0797890123", license_number: "LIC-1007", license_expiry: "2027-01-05", assigned_vehicle_plate: null, assigned_vehicle_id: null, assigned_route_name: null, assigned_route_id: null, status: "inactive", created_at: "2025-05-01" },
  { id: "DRV-0008", name_ar: "نادر سليم", phone: "0798901234", license_number: "LIC-1008", license_expiry: "2026-09-30", assigned_vehicle_plate: "و 2345", assigned_vehicle_id: "VEH-0006", assigned_route_name: "باص رقم ٥٠٠", assigned_route_id: "RT-006", status: "active", created_at: "2025-03-15" },
  { id: "DRV-0009", name_ar: "باسل عواد", phone: "0799012345", license_number: "LIC-1009", license_expiry: "2027-04-12", assigned_vehicle_plate: "ز 6789", assigned_vehicle_id: "VEH-0007", assigned_route_name: "باص سريع ١", assigned_route_id: "RT-001", status: "active", created_at: "2025-04-20" },
  { id: "DRV-0010", name_ar: "زياد عمرو", phone: "0790123456", license_number: "LIC-1010", license_expiry: "2025-10-08", assigned_vehicle_plate: null, assigned_vehicle_id: null, assigned_route_name: null, assigned_route_id: null, status: "inactive", created_at: "2025-06-01" },
];
      ...
      {Array.from({ length: cols }).map((_, i) => (
        <td key={i}>
          <div
            ...
            style={{ width: i === 4 ? "72px" : `${Math.random() * 80 + 60}px` }}
          />
        </td>
      ))}
    </tr>
    ...
    >
      ...
      >
        <div className="flex items-center gap-3 mb-5">
          ...
          <div>
            <h3 className="text-lg font-bold text-text-primary">تعيين مركبة</h3>
        ...
        <select
          ...
          className="w-full h-10 px-3 rounded-input bg-surface-2 border border-border text-sm text-text-primary focus:outline-none focus:border-brand-blue focus:ring-1 focus:ring-brand-blue/20 transition-all cursor-pointer appearance-none mb-6"
          style={{
            backgroundImage:
...
export default function DriversPage() {
  ...
  // ── Data State (simulated API with mock data) ────────────────────────────
  ...
  // Check if a license is about to expire (within 90 days)
    ...
    <DashboardShell headerProps={headerProps}>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {/* Total */}
          <div className="flex items-center gap-3 p-4 rounded-card bg-surface border border-border">
          ...
          <div className="flex items-center gap-3 p-4 rounded-card bg-surface border border-border">
            ...
            <div>
              <div className="text-xs font-medium text-text-secondary">نشط</div>
              <div className="text-xs font-semibold text-brand-green">
                {drivers.length > 0
                  ? ((statusStats.active / drivers.length) * 100).toFixed(0)
                  : 0}
                %
              </div>
            </div>
          ...
          {/* Inactive */}
          <div className="flex items-center gap-3 p-4 rounded-card bg-surface border border-border">
            <div className="w-10 h-10 rounded-input bg-text-tertiary/10 flex items-center justify-center text-lg font-bold text-text-tertiary">
              {statusStats.inactive}
            </div>
            <div>
              <div className="text-xs font-medium text-text-secondary">متوقف</div>
              <div className="text-xs font-semibold text-text-tertiary">
                {drivers.length > 0
                  ? ((statusStats.inactive / drivers.length) * 100).toFixed(0)
                  : 0}
                %
              </div>
            </div>
                      ...
                      >
                        ...
                        <td>
                          {driver.assigned_vehicle_plate ? (
                            <span className="text-sm font-mono text-text-primary bg-surface-2 px-2 py-0.5 rounded-input text-xs font-medium">
                              {driver.assigned_vehicle_plate}
                            </span>
                          ) : (
                            ...
                            <button
                              ...
                              className={`p-1.5 rounded-input transition-colors ${
                                driver.status === "active"
                                  ? "hover:bg-delayed/10 text-text-secondary hover:text-delayed"
                                  : "hover:bg-brand-green/10 text-text-secondary hover:text-brand-green"
                              }`}
                              title={
                                driver.status === "active"
                                  ? "إيقاف السائق"
        ...
        {!loading && drivers.length > 0 && (
          <div className="flex items-center justify-between text-xs text-text-tertiary">
            ...
            <div className="flex items-center gap-3">
              <span>
                {drivers.filter((d) => d.assigned_vehicle_id).length} سائق بمركبة
              </span>
              <button
                onClick={() => setDrivers([...MOCK_DRIVERS])}
              ...
              <div className="p-6 space-y-5">
                ...
                <div>
                  <label className="block text-xs font-medium text-text-secondary mb-1.5">
                    الاسم الكامل <span className="text-cancelled">*</span>
                  </label>
                  <input
                    value={form.name_ar}
                ...
                <div>
                  <label className="block text-xs font-medium text-text-secondary mb-1.5">
                    رقم الهاتف <span className="text-cancelled">*</span>
                  </label>
                  <input
                    type="tel"
                    value={form.phone}
